...
public class BFRegisterSet {
  ...
  /**
   * The array storing BigFraction values for registers.
   */
  private BigFraction[] registers; // registers

  /**
   * The number of available registers, from 'a' to 'z'.
   */
  private static final int NUM_REGISTERS = 26; // NUM_REGISTERS

  /**
   * Initialize the registers with 26 slots, 'a' through 'z'.
   */
  public BFRegisterSet() { // BFRegisterSet()
   ...
   * @throws IllegalArgumentException if the register is not between 'a' and 'z'.
   */
  public BigFraction get(char register) { // get
    int index = getRegisterIndex(register);
   ...
   * @throws IllegalArgumentException if the register is not between 'a' and 'z'.
   */
  private int getRegisterIndex(char register) { // getRegisterIndex(char register)
    if (register < 'a' || register > 'z') {
      throw new IllegalArgumentException("Register must be a letter between 'a' and 'z'.");
    } // if
    return register - 'a';
  } // getRegisterIndex(char register)
} // BFRegisterSet
